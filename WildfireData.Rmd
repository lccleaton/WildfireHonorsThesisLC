...
Then I wanted to see where only the longest evacuation durations occurred in California. (The top 10%)

```{r filter_out_na, eval = TRUE}
# Clean out NA fire names first
cali_wildfire_geo_time_clean <- cali_wildfire_geo_time %>%
  filter(!is.na(name))

# Compute 90th percentile threshold from CLEAN data
longest_threshold <- quantile(
  cali_wildfire_geo_time_clean$diff_hours,
  0.90,
  na.rm = TRUE
)

# Keep only the longest evacuations (top 10%)
cali_longest <- cali_wildfire_geo_time_clean %>%
  filter(diff_hours >= longest_threshold)

# Sanity check: should return 0
sum(is.na(cali_longest$name))
```


```{r longest_cali_threshold_heatmap, eval = FALSE}

leaflet(cali_longest) %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  addPolygons(
    fillColor = "red",
    fillOpacity = 0.8,
...
```{r longest_cali_threshold_chart, eval = TRUE}
cali_longest_df <- cali_longest %>%
  st_drop_geometry()

ggplot(cali_longest_df,
       aes(y = reorder(name, diff_hours),
           x = diff_hours)) +
  geom_point(color = "firebrick", size = 3) +
  labs(
    title = "Top 10% Longest Evacuations in California",
    subtitle = "Ranked by Fire",
    x = "Evacuation Duration (Hours)",
    y = "Fire Name"
  ) +
...
```{r cali_longest_table, eval = TRUE}
cali_longest_df %>%
  select(
    Fire = name,
    Zone = display_name,
